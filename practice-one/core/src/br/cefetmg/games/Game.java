...
public class Game extends ApplicationAdapter {
    ...
    private Goomba Gomba;
    private int int1 = 19;
    private int int2 = 22;
    public Game() {
    }
    //private Texture gombaSprite,gombaShadowSprite;
    ...
    public void create() {
        ...
        Gomba = new Goomba(new Texture("goomba.png"),new Texture("goomba-shadow.png"),new Texture("goomba-spritesheet.png"));
        //gombaShadowSprite = new Texture("goomba-shadow.png");
    ...
    public void render() {
        ...
        batch.begin();        
            ...
            batch.draw(Gomba.getGombaShadowSprite(),Gomba.getGoombax(),Gomba.getGoombay() - int1/3);
            if( Gomba.GombaState != 0)
                batch.draw( (TextureRegion) Gomba.animator(),Gomba.getGoombax(),Gomba.getGoombay());
            else
                batch.draw(Gomba.getGombaSprite(),Gomba.getGoombax(),Gomba.getGoombay());
     ...
     */
    public void manterNaTela(char a,float y){
        if(a=='x'){
            if(Gomba.getGoombax() + y  < Gdx.graphics.getWidth() - int1 && Gomba.getGoombax() + y  > 0 )
                Gomba.setGoombax(y);
        }else{
            if( (Gomba.getGoombay() + y)  < (Gdx.graphics.getHeight() - int2) && Gomba.getGoombay() + y  > 0 )
                Gomba.setGoombay(y);
        }
    }
    
    public void update(float delta) {
        Gomba.tempoDaAnimacao += Gdx.graphics.getDeltaTime();
